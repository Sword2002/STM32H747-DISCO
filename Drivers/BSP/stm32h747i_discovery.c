...
int32_t  BSP_PB_Init(Button_TypeDef Button, ButtonMode_TypeDef ButtonMode)
{
  ...
  gpio_init_structure.Speed = GPIO_SPEED_FREQ_LOW; // GPIO_SPEED_FREQ_HIGH
  ...
  for(joykey = 0U; joykey < JOY_KEY_NUMBER ; joykey++)
  {
    ...
    if((key_pressed & (uint32_t)JoyPins) == key_pressed)
    {
      ...
      /* Enable the JOY clock */
        ...
        gpio_init_structure.Pull = GPIO_PULLUP;          // GPIO_PULLUP GPIO_NOPULL
        gpio_init_structure.Speed = GPIO_SPEED_FREQ_LOW; // GPIO_SPEED_FREQ_VERY_HIGH
        ...
        else /*(JoyMode == JOY_MODE_EXTI)*/
        {
          ...
          gpio_init_structure.Mode = GPIO_MODE_IT_RISING;  // GPIO_MODE_IT_RISING_FALLING
          HAL_GPIO_Init(JOY1_PORT[joykey], &gpio_init_structure);
